...
import { 
  ...
  Gamepad2, Send, RefreshCw, Copy, Check, X, Loader2,
  Sun, Moon, Heart, Zap, DoorOpen, Reply, Bot
} from 'lucide-react';
      ...
      <div className="max-w-md w-full space-y-8 relative z-10">
        <div className="text-center space-y-2">
          <div className="relative inline-block mb-4">
             <div className="w-20 h-20 bg-gradient-to-br from-violet-500 to-fuchsia-500 rounded-[2rem] flex items-center justify-center shadow-2xl transform rotate-3 hover:rotate-6 transition-transform">
                <span className="text-4xl font-extrabold text-white">D</span>
             </div>
             <div className="absolute -bottom-2 -right-2 w-8 h-8 bg-white dark:bg-slate-800 rounded-full flex items-center justify-center shadow-lg text-rose-500">
               <Heart size={16} fill="currentColor" />
             </div>
          </div>
          <h1 className="text-4xl font-extrabold text-black dark:text-white tracking-tight">DuoSpace</h1>
          <p className="text-slate-600 dark:text-slate-300 font-medium">Your private world for two.</p>
        </div>

        <Card className="backdrop-blur-xl">
          <div className="flex bg-slate-100 dark:bg-slate-800/50 p-1.5 rounded-2xl mb-8">
            <button 
              className={`flex-1 py-2.5 text-sm font-bold rounded-xl transition-all ${mode === 'signup' ? 'bg-white dark:bg-slate-700 shadow-md text-violet-600 dark:text-white' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400'}`}
              onClick={() => setMode('signup')}
            >
              Sign Up
            </button>
             <button 
              className={`flex-1 py-2.5 text-sm font-bold rounded-xl transition-all ${mode === 'login' ? 'bg-white dark:bg-slate-700 shadow-md text-violet-600 dark:text-white' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400'}`}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Button 
              type="submit" 
              className="w-full shadow-violet-500/25" 
    ...
    <div className="min-h-full p-6 pb-24 relative">
      <div className="max-w-xl mx-auto space-y-8 relative z-10">
        <header className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-black text-black dark:text-white">Your Spaces</h1>
...
const MiniGame = ({ spaceId, user }: { spaceId: string, user: User }) => {
  ...
  const handleMove = async (index: number) => {
    if (!activeGame) {
       ...
       await API.updateGame(spaceId, {
         ...
         type: 'tictactoe',
         board: newBoard,
         currentPlayer: opponentId || 'unknown', 
         status: 'active',
         resetRequests: [],
    ...
    await API.updateGame(spaceId, {
      ...activeGame,
      board: newBoard,
      currentPlayer: opponentId || user.id, 
      lastUpdated: Date.now()
    });
  };
  ...
  const requestReset = async () => {
    ...
    if (newRequests.length >= (space?.members.length || 1)) {
       await API.updateGame(spaceId, {
         id: API.generateId(),
         type: 'tictactoe',
       ...
       await API.updateGame(spaceId, { ...activeGame, resetRequests: newRequests });
    }
  };
      ...
      <div className="grid grid-cols-3 gap-3 p-4 bg-white dark:bg-slate-800 rounded-[2rem] shadow-xl shadow-violet-500/10">
        {board.map((cell, i) => (
          <button 
            ...
            className="w-20 h-20 bg-slate-50 dark:bg-slate-700/50 rounded-2xl flex items-center justify-center text-4xl font-black text-slate-700 dark:text-white shadow-inner hover:bg-violet-50 dark:hover:bg-violet-900/20 transition-colors disabled:opacity-80"
          >
  ...
  return (
    <div className="p-4 space-y-6 h-full overflow-y-auto no-scrollbar bg-slate-50/50 dark:bg-slate-900/50">
      ...
      <div className="space-y-4 pb-20">
        ...
        {songs.map(song => (
            <div key={song.id} className="bg-white dark:bg-slate-800 p-3 rounded-2xl flex gap-4 shadow-sm border border-slate-100 dark:border-slate-700 items-center">
...
const SettingsPanel = ({ spaceId, user }: { spaceId: string, user: User }) => {
  ...
  return (
    <div className="p-6 space-y-6 h-full overflow-y-auto bg-slate-50/50 dark:bg-slate-900/50">
      <Card>
        ...
        <div className="space-y-4">
          <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl flex items-center justify-between border border-slate-100 dark:border-slate-700">
             ...
             <button className="w-10 h-10 bg-white dark:bg-slate-700 rounded-full flex items-center justify-center text-slate-400 shadow-sm"><Copy size={18} /></button>
          </div>
        </div>
      </Card>
      ...
      <Button variant={confirmLeave ? "danger" : "secondary"} className="w-full mt-8" onClick={handleLeave} isLoading={leaving}>
        {confirmLeave ? "Are you sure? Click again." : <><DoorOpen size={18} /> Leave Space</>}
      </Button>
    </div>
  );
};
